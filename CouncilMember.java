...
public class CouncilMember implements Runnable{
    ...
    public static void writeOutputToTestFile(ArrayList<CouncilMember> memberList, String outputFileLocation){
        ...
        for(int i = 0; i < memberList.size(); i++){
            ...
            // if the value is in the table, increment counter
            ...
            // if the value is new, add value to table with counter = 1
            else{
                acceptedValueCounterTable.put(acceptedValue, 1); 
            }
        }
        int highestCounter = 0; 
        String valueWithHighestCounter = ""; 
        // go through the counter table to get the value that being agreed by the majority (with the highest acceptance)
    ...
    public static void main(String args[]){
        ...
        // in the middle, kill all the members in the protocol (including 3 Proposers)
        // so that all the members will not get any request from Proposers
        // then go back online after the timeout (timeout is 12s, bring them back after 13 seconds just for sure) 
        // we can see how the proposers will handle the scenario when they cannot receive any response within the timeout
        if(args[0].equals("testing") && args[1].equals("14")){
            ...
            System.out.println("Kill all members in the protocol."); 
            // go back online after 13 seconds
            try{
                Thread.sleep(13000); 
            }
            catch(Exception e){
